...
void Orchestrator::calculateExpectedPolicy(TPML_PCR_SELECTION* selection, unsigned char* pcrDigest, unsigned char* result)
{
	...
	UINT16 written = 0;
	TSS_TPML_PCR_SELECTION_Marshal(selection, &written, &buffer, NULL);
...
{
	return (pcr < IMPLEMENTATION_PCR && ((selection->pcrSelect[pcr / 8])& (1 << (pcr % 8))) != 0);
}
